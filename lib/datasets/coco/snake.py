import os
from lib.utils.snake import snake_voc_utils, snake_coco_utils, snake_config, visualize_utils
import cv2
import numpy as np
import math
...
class Dataset(data.Dataset):
    ...
    def read_original_data(self, anno, path): ## anno为process_info来的
        # print(path)
        assert os.path.exists(path)
        img = cv2.imread(path)
        # if(type(anno[0]['segmentation'])==dict):
        #     for obj in anno:
        #         t=mask_utils.decode(obj['segmentation'])
        #         obj['segmentation']=data_utils.polygonFromMask(t)
