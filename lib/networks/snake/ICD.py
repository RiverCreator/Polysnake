...
class RAFT(nn.Module):
    ...
    def evolve_poly(self, snake, cnn_feature, i_it_poly, c_it_poly, ind, box_pred, vis_pred=None):  # i_it_poly为init point，c_it_poly为相对init point，ind为标注ct为batch中哪个图片的
        if len(i_it_poly) == 0:
        ...
        # init_feature = init_feature*(1 + vis_probs.sigmoid())
    ...
    def forward(self, output, cnn_feature, fine_feature, batch):
        ...
        if batch is not None and 'test' not in batch['meta']:
            ...
            #box_mask_pred, vis_mask_pred, roi = self.box_mask_head(cnn_feature, poly_init, batch['ct_01'].byte())
        ...
        if not self.training:
            with torch.no_grad():
                ...
                #box_mask_pred, vis_mask_pred, roi = self.box_mask_head(cnn_feature, poly_init, detection=detection)
                box_mask_pred, roi = self.box_mask_head(cnn_feature, poly_init, detection=detection)
