...
from .utils import patch2masks
class PatchDCT(nn.Module):
    def __init__(self,input_shape, *, num_classes, conv_dims, conv_norm="", dct_vector_dim, mask_size, hidden_features,
                 fine_features_resolution,mask_size_assemble, patch_size, patch_dct_vector_dim, 
                 mask_loss_para, dct_loss_type, patch_threshold,eval_gt, **kwargs) -> None:
        """
        ...
        #self.num_stage = num_stage - 1 只用其中一个阶段
        #self.loss_para = mask_loss_para_each_stage #TODO loss的权重到时候手动改下
        ...
        self.conv_norm_relus = nn.ModuleList() #必须使用这个，使用list，Dataparallel不能正确分配cuda 
    ...
    #TODO 这里的fine_mask_features原论文中用的FPN中的P2，pooler为42 42大小的
    def layers(self, x, fine_mask_features):
        ...
        """
        for layer in self.conv_norm_relus:
            #layer=layer.to(device=x.device)
        ...
        if not self.training:
            num_masks = x.shape[0]
            if num_masks == 0:
                return x, torch.empty((0,self.num_classes, 3, self.scale, self.scale),dtype=torch.float32).cuda(), torch.empty((0,self.num_classes, self.patch_dct_vector_dim, self.scale, self.scale),dtype=torch.float32).cuda()
    ...
    def stage_patch2mask(self, bfg, patch_vectors):
        device = bfg.device
        index = torch.argmax(bfg, dim=1) #求出最大值的下标，就表示其对应的类别 0是前景 1是边缘 2是背景
    ...
    def forward(self, x, fine_mask_features, batch = None, detection = None):
        """
        Args:
        ...
        """
        #self=self.to(device=x.device)
        x, bfg, patch_vectors = self.layers(x, fine_mask_features) # x为encode后的dct feature
        if batch is not None:
            #x需要decode变成mask
            gt_classes = batch['ct_cls'][batch['ct_01'].byte()]
            num_instance = gt_classes.size()[0]
            indices = torch.arange(num_instance).to(gt_classes.device)
            bfg = bfg[indices, gt_classes].permute(0, 2, 3, 1).reshape(-1, 3) #取出对应classes的预测 将bfg预测的类别作为第1维
            patch_vectors = patch_vectors[indices, gt_classes].permute(0, 2, 3, 1).reshape(-1, self.patch_dct_vector_dim) #同样展开
            pred_masks=self.stage_patch2mask(bfg, patch_vectors)
            return x, bfg, patch_vectors, pred_masks
        else:
            pred_classes = detection[:,3].long()
            num_masks = pred_classes.size()[0]
            indices = torch.arange(num_masks).cuda() #torch.Size([10, 60, 3, 14, 14])
            if bfg.numel() == 0:
                print("bfg error")
            try:
                # if bfg.numel() == 0:
                #     bfg = torch.empty((0,3),dtype=bfg.dtype).cuda()
                #     patch_vectors = torch.empty((0,self.patch_dct_vector_dim),dtype=patch_vectors.dtype).cuda()
                # else:
                bfg = bfg[indices, pred_classes].permute(0, 2, 3, 1).reshape(-1, 3) # 1960 3
                patch_vectors = patch_vectors[indices, pred_classes].permute(0, 2, 3, 1).reshape(-1,self.patch_dct_vector_dim)
            except:
                print("error!")
            #patch_vectors = patch_vectors[indices, pred_classes].permute(0, 2, 3, 1).reshape(-1,self.patch_dct_vector_dim)
            #with torch.no_grad(): 3 60 3 14 14 => 3 1 6 14 14 
            ...
            if patch_vectors.numel() == 0:
                return x, bfg, patch_vectors, torch.empty((0,1,self.mask_size_assemble,self.mask_size_assemble))
            pred_mask_rc = self.patch_dct_encoding.decode(patch_vectors).real
            ...
            pred_mask_rc = pred_mask_rc[:, None, :, :] # 3 1 112 112 
            return x, bfg, patch_vectors, pred_mask_rc
        if self.training:
    ...
    def mask_rcnn_dct_loss(self, pred_mask_logits, bfg, patch_vectors, gt_masks, vis_period=0):
        """
        ...
        if self.num_stage == 1:
            ...
            mask_loss_2 = F.cross_entropy(bfg, gt_bfg) #bfg为[N,3],3即是三个类别 gt_bfg就是标注的哪个样本是属于哪个类别，因为是划分了patch，这里起始就是将经过dct转换的patch进行分类
