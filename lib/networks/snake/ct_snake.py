...
class Network(nn.Module):
    ...
    def forward(self, x, batch=None,more_info=None):
        ...
        output, cnn_feature, fine_feature = self.dla(x) # dla输出的output为ct_heatmap,point wh offset,boundry mask,这里输出的cnn_feature为x经过4倍降采样的(而且是经过多个尺度特征增强的，)
        output = self.raft(output, cnn_feature, batch, fine_feature,more_info = more_info) #这里的batch
        # if(cfg.use_interactive):
        #     output = self.raft(output, cnn_feature, batch, fine_feature,more_info = more_info) #这里的batch
        # else:
        #     output = self.raft(output, cnn_feature, batch, fine_feature) #这里的batch
        return output
