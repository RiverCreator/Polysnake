...
def load_model(net, optim, scheduler, recorder, model_dir,resume=True, epoch=-1,from_best=False,best_val=None):
    if not resume:
...
def save_model(net, optim, scheduler, recorder, epoch, model_dir,model_name=None,ap=None):
    ...
    torch.save({
        ...
        'epoch': epoch,
        'ap': ap
    }, os.path.join(model_dir, '{}.pth'.format(model_name)))
...
def load_network(net, model_dir, resume=True, epoch=-1, strict=True):
    ...
    pretrained_model=torch.load(os.path.join(model_dir,'214.pth'))
