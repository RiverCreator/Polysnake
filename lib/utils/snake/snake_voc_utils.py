...
def uniformsample(pgtnp_px2, newpnum):
    ...
    pgtnext_px2 = pgtnp_px2[idxnext_p] # 循环移动一下，这样用来计算与顺时针相邻的点之间的距离
    edgelen_p = np.sqrt(np.sum((pgtnext_px2 - pgtnp_px2) ** 2, axis=1))## 计算边长
    edgeidxsort_p = np.argsort(edgelen_p) ## 边长度排序（从小到大），存放排序的下标

    # two cases
    # we need to remove gt points
    # we simply remove shortest paths
    if pnum > newpnum: #如果点过多
    ...
    # we simply add it uniformly
        ...
        edgenum = np.round(edgelen_p * newpnum / np.sum(edgelen_p)).astype(np.int32)  # 这里先计算一些每个边进行采样的个数，这里是均匀采样
        for i in range(pnum): ##避免计算下取整而变为0，让其多采样为1个
            if edgenum[i] == 0:
                edgenum[i] = 1

        # after round, it may has 1 or 2 mismatch
        edgenumsum = np.sum(edgenum)
        if edgenumsum != newpnum: ## 不对的话则处理一下

            if edgenumsum > newpnum:

                id = -1
                passnum = edgenumsum - newpnum ##多了多少个点需要忽略，然后将长边的点优先忽略
                while passnum > 0:
        ...
        psample = []
        for i in range(pnum):
            pb_1x2 = pgtnp_px2[i:i + 1]  #begin point
            pe_1x2 = pgtnext_px2[i:i + 1] #end point

            pnewnum = edgenum[i] #当前边要采样的点数
            wnp_kx1 = np.arange(edgenum[i], dtype=np.float32).reshape(-1, 1) / edgenum[i] ## 计算每段边所在的比例位置

            pmids = pb_1x2 * (1 - wnp_kx1) + pe_1x2 * wnp_kx1 #插值方法，直接计算出中间所有点的坐标
