...
def polygonFromMask(maskedArr): # https://github.com/hazirbas/coco-json-converter/blob/master/generate_coco_json.py
    #contours= cv2.findContours(maskedArr_copy, cv2.RETR_TREE, cv2.CHAIN_APPROX_SIMPLE)
    #fortran_ground_truth_binary_mask = np.asfortranarray(maskedArr)
    ...
    for contour in contours:
        ...
        if len(contour) >= 18:
            ...
            #     coords.append(point[0][1])
            segmentation.append(contour.flatten().tolist())
            #segmentation.append(coords)

    return segmentation #, [x, y, w, h], area
## 这里使用的centernet的函数，计算高斯函数的主要思想就是，让GT框的角点周围一定范围内且iou在0.7范围内，认为其为正样本
def gaussian_radius(det_size, min_overlap=0.7):
    height, width = det_size
    ## 这里分成了三种情况，第一种情况为一个角点在GT框内，另一个角点在GT框外
    a1 = 1
    b1 = (height + width)
    ...
    if b3 ** 2 - 4 * a3 * c3 < 0:
        ...
        r3 = (b3 + sq3) / 2
    return min(r1, r2, r3) ## 取最小的半径，因为每个求得的半径都是如果再大一点的话，就不满足IoU=0.7了，因此需要满足这三种情况下IoU>=0.7的最小半径
